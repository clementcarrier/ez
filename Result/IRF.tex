...
\hlstd{end}\hlkwb{<-}\hlkwd{subset}\hlstd{(difdata[,}\hlopt{-}\hlkwd{which}\hlstd{(}\hlkwd{names}\hlstd{(difdata)} \hlopt{%in%} \hlkwd{c}\hlstd{(}\hlstr{"POILU"}\hlstd{))])}
